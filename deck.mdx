...
<Notes>
  ...
  <p>
    <strong>API (Application Programming Interface):</strong> JAMstack sites use APIs to integrate data and other dynamic information, often as JSON, with your front end (headless CMS)</p>
  <p>
    <strong>Markup:</strong> HTML / presentation layer
  </p>
  <hr />
  <h2>Decoupled web</h2>
  <p>JAMstack === decoupled approach to web dev</p>
  ...
  <p>The name was coined bc "static site" doesn't do it justice (comment systems, web forms, ecommerce...)</p>
  <hr />
...
<Notes>
  ...
  <p>Once we get the data we need, the db server will send the site back through a web server where an HTML page with our dynamic data will get assembled</p>
  ...
  <p>Data - or content - may live in a headless CMS, or in markdown files in the repo, or pulled in via other APIs</p>
  <p>Developer eats that server "slowness" in a build step (also happens during deploy)</p>
  <p>Lucky for us, that build step can be automated w CICD (continuous integration & continous delivery)</p>
...
# <span className="highlight">JAM</span>stack Ingredients
...
<Notes>
  <h2>Static site generator</h2>
  <p>Static - lacking in movement or change; we're talking about static architecture (host statically), not static experience</p>
...
<Notes>
  ...
  <p>Gatsby has clear documentation + starters, which are Gatsby sites that are preconfigured for different use cases to give you a head start for your project.</p>
  <p>Built with performance and accessibility in mind</p>
  <p>(no more clicks) So, once we have this Gatsby site, what are we going to do with it?</p>
...
<Notes>
  <p>When our site is deployed, Netlify's post-processing bots are going to look for `netlify` attributes on your form in order to capture it</p>
  <p>These bots only know how to parse HTML, so we have to give them a little help to make sure they can detect forms rendered with Javascript.</p>
  <p>Open the Contact.js file in your favorite IDE</p>
</Notes>
...
<Notes>
  ...
  <p>
    We can visit our Netlify dashboard, click Forms, and see and manage the
    entries
  </p>
...
<Notes>
  <p>Why does all this matter again?</p>
  <p>JAMstack doesn't mean you're stuck with a "static," unchanging site</p>
  <p>It means you're choosing all of these advantages - performance, scaling, a better dev experience</p>
  <p>by choosing a static architecture</p>
</Notes>
